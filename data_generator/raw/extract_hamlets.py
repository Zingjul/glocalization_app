...
OUTPUT_FILE = "extracted_hamlets.json"
...
for country_code in os.listdir(BASE_DIR):
    ...
    ndjson_path = os.path.join(sub_folder, "place-hamlet.ndjson")  # 👈 Switched to place-hamlet.ndjson
    ...
    try:
        with open(ndjson_path, "r", encoding="utf-8") as file:
            for line in file:
                ...
                hamlet_name = data.get("other_names", {}).get("name:en") or data.get("name")
                state_name = data["address"].get("state")
                country_id = data["address"].get("country_code")

                if not hamlet_name or not state_name or not country_id:
                    continue

                hamlet_name = smart_translate(hamlet_name)
                ...
                hamlets_data.append({
                    "name": hamlet_name,
                    "state": state_name,
                    "type": "hamlet",  # 👈 Default type set to 'hamlet'
...
if hamlets_data:
    try:
        with open(OUTPUT_FILE, "w", encoding="utf-8") as json_file:
            json.dump(hamlets_data, json_file, indent=4, ensure_ascii=False)
        print(f"✅ Saved translated hamlet data to '{OUTPUT_FILE}'")
    except Exception as e:
        print(f"❌ Error saving JSON file: {e}")
else:
    print("⚠️ No valid hamlet data extracted!")
