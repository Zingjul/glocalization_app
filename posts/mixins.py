...
class LocationFieldsSetupMixin:
    ...
    def init_location_queryset(self):
        # 🛡 Ensure dropdowns fall back to UNSPECIFIED (id=1) instead of empty string
        continent_id = self.data.get("post_continent") or 1
        country_id = self.data.get("post_country") or 1
        state_id = self.data.get("post_state") or 1

        self.fields["post_continent"].queryset = Continent.objects.all().order_by("id")
        self.fields["post_country"].queryset = Country.objects.filter(continent_id=continent_id).order_by("id")
        self.fields["post_state"].queryset = State.objects.filter(country_id=country_id).order_by("id")
        self.fields["post_town"].queryset = Town.objects.filter(state_id=state_id).order_by("id")

        # 🚫 Remove Django's blank <option> and default to Unspecified (id=1)
