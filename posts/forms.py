...
class ProductPostForm(forms.ModelForm, ImageFieldsMixin, LocationFieldsSetupMixin):
    ...
    def save(self, commit=True):
        ...
        cleaned_data = self.cleaned_data  # Optional but explicit
...
class LaborPostForm(forms.ModelForm, ImageFieldsMixin, LocationFieldsSetupMixin):
    ...
    post_town_input = forms.CharField(
        required=False,
        label="If your town isn't listed, enter it here"
    )
...
# this code is used to force django not to use its defualt empty string on dropdown options when a page loads, rather it is set to make use of a preselected id of 1 
        for field in ["post_continent", "post_country", "post_state", "post_town"]:
            if field in self.fields:
                self.fields[field].empty_label = None  # Remove Django default blank option
                self.fields[field].queryset = self.fields[field].queryset.order_by("id")
                try:
                    self.initial[field] = 1  # Set default to id=1 (Unspecified)
                except Exception:
                    pass  # Fail silently if initial setting causes issues

    def clean(self):
        cleaned_data = super().clean()

        # Auto-create town if needed
        town_input = cleaned_data.get("post_town_input")
        town_fk = cleaned_data.get("post_town")

        if not town_fk and town_input:
            from location.models import Town
            town_obj, _ = Town.objects.get_or_create(
                name=town_input.strip(),
                state=cleaned_data.get("post_state"),
                country=cleaned_data.get("post_country"),
                continent=cleaned_data.get("post_continent"),
                defaults={"code": "custom"}
            )
            cleaned_data["post_town"] = town_obj
